...
export const cleanUser = (this_) => {
  this_.state.user.isKycUser = false
  this_.state.user.isGatewayUser = false
  this_.state.user.isCarrierUser = false
  this_.state.user.kycToken = ''
  this_.state.user.gatewayToken = ''
  this_.state.user.kycImagePath = ''
  this_.state.user.kycStatus = ''
  this_.state.user.kycCountry = []
  this_.state.user.kycUrl = ''
  this_.state.user.gatewayUrl = ''
}
export const loginToPath = (ZOSIn, thisVm, loginPath) => {
  let sLogin = !ZOSIn.accountIsLoginout()
  thisVm.$store.state.login = sLogin
  if (!sLogin) {
    cleanUser(thisVm.$store)
...
export const formatDbDate = (dbString) => {
  if (dbString.indexOf(':') > 0) {
    dbString.replace(' ', 'T')
    return getTimezoneDate(dbString)
  } else if (Number(dbString) > 1000000000000) {
    return getTimezoneDate(formatDate(new Date(Number(dbString))))
  }
}
