<template>
  <div slot="content" class="container" v-loading="loading" :element-loading-text="$t('m.Frame.loading')" style="min-height: 10vw;">
    <div v-if="!loading">
      <!--邀请方式-->
      ...
      <ul class="basicinfos">
        ...
        <el-dialog
          ...
          <span slot="footer" class="dialog-footer">
              <!-- 确定 -->
              <el-button type="primary" @click="copyDialog = false">{{$t('m.Setting.sure')}}</el-button>
            </span>
        </el-dialog>
      </ul>
    </div>
    <div v-else class="un-login">
      ...
      <el-button type="primary" @click="login">{{$t('m.Frame.login')}}</el-button>
    </div>
    <h1>
      <!--活动细则-->
