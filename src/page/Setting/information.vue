<template>
  <div slot="content" v-loading="loading" :element-loading-text="$t('m.Frame.loading')" style="min-height: 10vw;">
    <div v-show="!loading">
        <h1 class="f16">
          <!--基本信息-->
          {{$t('m.Setting.basic.title')}}
        </h1>
        ...
        <el-form :model="passwordForm"
          ...
          <el-form-item>
            <ul style="line-height: 24px">
              <li class="feezos margin-t10">
                <!--燃料费-->
                <!--(约)-->
                <div>{{$t('m.Setting.fuelCost')}}: ZOS {{passwordForm.feeZos}} {{$t('m.transfer.about')}}</div>
          ...
          <el-form-item>
            <p class="text-right margin-t10">
              ...
              ></y-button>
              <y-button :text="$t('m.Setting.sure')"
      ...
      >
        <el-form :model="mForm" status-icon :rules="mRules" ref="mForm" label-width="100px" class="demo-ruleForm"
                 ...
                 :element-loading-text="$t('m.Setting.submiting')">
          <!--手机号-->
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
            ...
            <y-button :text="$t('m.Setting.sure')" @btnClick="couponVisible = false;" class="main-btn"></y-button>
          </span>
      </el-dialog>
    </div>
  </div>
  ...
  export default {
    ...
    data () {
      const validatePassOld = (rule, value, callback) => {
        if (value === '') {
          // '请输入密码'
          callback(new Error(this.$t('m.Setting.inputPassword')))
        } else {
          callback()
        }
      }
    ...
    methods: {
      ...
      _adminNotify () {
        if (this.$store.state.adminNotifyId && this.$store.state.adminNotifyId !== '') return
