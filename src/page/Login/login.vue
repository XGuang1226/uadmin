<template>
  ...
  <div v-loading.fullscreen.lock="mainloading" :element-loading-text="$t('m.Frame.loading')" style="min-height: 100vh;">
  <div class="login v2">
    <div class="wrapper">
      <div class="dialog dialog-shadow" style="display: block; margin-top: -320px;">
        <div class="title">
          <div></div>
          <h4>{{$t("m.Frame.loginTitle")}}</h4>
        </div>
        <div v-if="loginPage" class="content">
          <ul class="common-form">
            ...
            <li style="text-align: right" class="pr">
              ...
              <a href="javascript:;" class="register" @click="toRegister">{{$t("m.Frame.registerAccount")}}</a>
          ...
          <div style="margin-top: 25px; display: flex; flex-direction: column">
            <y-button :text="$t('m.Frame.login')"
                      ...
                      style="flex:1"
                      ></y-button>
            <y-button :text="$t('m.Frame.home')" @btnClick="login_back" class="margin-t10"></y-button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    // Ella  无效状态有点问题
    isDisabled () {
      if ((this.ruleForm.userPwd && this.ruleForm.userName && this.logintxt === this.$t('m.Frame.login')) && this.disabledRegs && this.validName) {
        return true
      } else {
        return false
      }
