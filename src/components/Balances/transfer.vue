    ...
    <div>
          ...
          <div v-loading="loading" :element-loading-text="$t('m.Setting.applyWaiting')" style="min-height: 10vw;">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
              ...
              <p class="feel margin-l20">
                  <!--燃料费-->
                  <!--(约)-->
                  <span>{{$t('m.Setting.fuelCost')}}: {{feel}} ZOS {{$t('m.transfer.about')}}</span>
        ...
        <div v-loading="loadingsCon" :element-loading-text="$t('m.transfer.being')" style="min-height: 10vw;">
            <span>
                ...
                <p>{{$t('m.Setting.fuelCost')}}: {{feel}} ZOS</p>
                </div>
            </span>
            <div slot="footer" class="dialog-footer" style="text-align: right;">
            <!--取 消-->
            <el-button @click="transferCannel">{{$t('m.Setting.cancel')}}</el-button>
            <!--确 定-->
            <el-button type="primary" @click="transferConfrom('ruleForm')">{{$t('m.Setting.sure')}}</el-button>
            </div>
        </div>
        </el-dialog>
    ...
    transferConfrom (formName) {
      ...
      ZOSInstance.broadcastTransaction(this.tr).then(res => {
        ...
        MessageBox.alert(this.$t('m.transfer.transferSucc'), {
          // '确定'
          confirmButtonText: this.$t('m.Setting.sure')
        })
        this.$refs[formName].resetFields()
        this.ruleForm.mome = ''
        this.visible = false
